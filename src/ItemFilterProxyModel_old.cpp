...
int ItemFilterProxyModel::rowCount(const QModelIndex &parentIndex) const
{
    ...
    return it == m_proxyChildrenHash.end() ? 0 : it->second.size();
}
...
QModelIndex ItemFilterProxyModel::mapToSource(const QModelIndex &proxyIndex) const
{
    ...
    if (it == m_proxyIndexHash.end()) {
        for (auto i = m_proxyIndexHash.begin(); i != m_proxyIndexHash.end(); ++i) {
            qDebug() << i->first;
        }
        return {};
    }
    auto& sourceIndex = it->second._sourceIndex;
    // Refresh source Index
    sourceIndex = sourceModel()->index(sourceIndex.row(), sourceIndex.column());
    return sourceIndex;
}
    ...
    for (int row = 0; row < rootRowCount; ++row) {
        ...
        for (int col = 0; col < rootColumnCount; ++col) {
            ...
            if (isRootNodeVisible) {
                ...
                m_proxyIndexHash.emplace(proxyIndex, ProxyModelIndexInfo{ sourceIndex, QModelIndex() });
        ...
        for (auto childSourceRow = 0; childSourceRow < childCount; ++childSourceRow) {
            ...
            for (auto childSourceCol = 0; childSourceCol < columnCount; ++childSourceCol) {
                ...
                if (isChildVisible) {
                    ...
                    m_proxyIndexHash.emplace(childProxyIndex, ProxyModelIndexInfo{ childSourceIndex, proxyParent });
